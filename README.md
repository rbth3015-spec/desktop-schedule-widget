...
### 체크리스트 (루틴)
- 운동처럼 **매일·매주 하는 일**은 달력에 그리지 않고 오른쪽 `체크리스트` 에만 모은다.
  매일 하는 일을 달력에 매일 막대로 그리면 정작 약속이 묻힌다.
- 목록 위 **`체크리스트`** 버튼으로 만든다. 반복(매일)과 '달력에 표시 안 함' 이 미리 켜져
  있으므로 이름만 적으면 된다. 약속과는 다른 물건이라 입구를 따로 냈다 —
  '반복 켜고 → 더보기 펼치고 → 체크리스트로만 누르고' 를 매번 거치는 건 너무 멀었다.
- 매주 반복은 **요일을 고를 수 있다** — '월수금 운동'. 고르지 않으면 시작일의 요일을 따른다.
- 체크는 회차마다 따로다. 오늘 체크해도 내일 것은 그대로 남는다.
- 그날 것을 다 하면 제목이 `체크리스트 · 오늘 다 했어요` 로 바뀐다.
