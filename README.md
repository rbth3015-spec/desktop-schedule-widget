...
- 머리글의 **`24 오늘`** 버튼은 오늘이 며칠인지 함께 알려 준다. 이미 오늘이면 흐려진다.
  검색은 평소 돋보기 하나로 접혀 있다가 누르면 펼쳐진다 — 그 자리를 날짜에 내준다.
...
- **한 일정을 펼치면 나머지는 접힌다.** 오른쪽 패널은 좁아서 상세와 목록이 자리를 다투면
  ...
  `‹ 목록으로` 또는 `Esc` 로 돌아온다.
- 상세 맨 위에 **이름 칸**이 있다. 예전에는 '제목 더블클릭' 이 유일한 길이라
  펼쳐 놓고도 이름을 못 고치는 자리였다. 더블클릭도 그대로 쓸 수 있다.
- 드래그 정렬, 태그 필터.
