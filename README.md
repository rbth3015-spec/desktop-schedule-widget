...
- **막대를 잡아 옮기고, 양 끝을 잡아 기간을 늘리고 줄인다.**
  끄는 동안 어디로 가는지 칸에 칠해 보여 주고 `8/15(토) → 8/17(월) · 3일간` 배지가 따라온다.
  HTML5 드래그 대신 마우스로 직접 다룬다 — 그래야 미리보기와 기간 조절을 한 방식으로 다룰 수 있다.
  (반복 일정은 규칙 하나를 공유하므로 캘린더에서 옮길 수 없고, 그 이유를 알려 준다)
...
- **언젠가는 꺼내 쓰는 자리다.** 날짜를 정하기 애매한 일을 가볍게 적어 두고,
  항목에 커서를 올리면 뜨는 `오늘` `내일` `주말` 을 한 번 누르면 그날로 잡힌다.
  일주일이 넘게 묵은 항목에는 `2주째` `3달째` 가 붙어, 잡거나 지울 때가 됐음을 알린다.
  (예전에는 캘린더로 끌거나 상세를 펼쳐야 해서, 넣기만 하고 안 나오는 자리였다)
- 검색은 평소 **`🔍 검색`** 버튼으로 접혀 있다가 누르면 펼쳐진다. 줄 하나를 아낀다.
  (오늘로 가는 버튼은 캘린더 머리글의 `오늘로` 하나뿐이다 — 같은 일을 하는 버튼을 둘 두지 않는다)
...
- 폼은 **스크롤 없이 한 화면에** 들어온다. 날짜 칩 / 시작 / 종료 / (길이 칩 + 요약) 네 줄,
  색과 중요도는 한 줄에 나란히. 반복·알림·태그·링크는 '더보기' 에 접어 둔다.
...
- **한 일정을 펼치면 나머지는 접힌다.** 오른쪽 패널은 좁아서 상세와 목록이 자리를 다투면
  둘 다 못 쓰게 된다. 고치는 동안에는 패널을 통째로 상세에 내주고, 머리글의
  `‹ 목록으로` 또는 `Esc` 로 돌아온다.
- **펼친 항목의 제목이 그대로 입력칸이 된다.** 눈에 보이는 그 글자를 바로 고치므로
  '이름' 칸을 따로 두지 않는다(같은 값이 화면에 두 번 나오지 않는다).
  접혀 있을 때는 그냥 글자라, 제목째 잡아 캘린더로 끌 수 있다.
- 드래그 정렬, 태그 필터.
...
- **전역 검색** — 검색창은 날짜와 무관하게 전체에서 찾는다. 다른 달의 일정도 잡힌다.
  결과에는 **날짜가 함께 붙는다**(`8/26(수)`). 제목만 있으면 언제 일인지 알 수 없어
  눌러 보기 전에는 고를 수가 없다. 누르면 상세가 열리면서 **달력도 그 날짜로 따라간다.**
...
### 공휴일
- 달력에 **대한민국 공휴일**을 표시한다. 대체공휴일·임시공휴일까지 포함한다.
- 자료는 우주항공청 월력요항을 정리한 공개 데이터를 **메인 프로세스가 받아** 온다.
  렌더러는 CSP 가 `default-src 'none'` 이라 네트워크를 쓸 수 없고, 그대로 두는 게 맞다.
- 받은 자료는 `userData/holidays/<연도>.json` 에 남긴다. 다음부터는 캐시로 즉시 그리고
  14일이 지난 것만 조용히 다시 받는다. **네트워크가 없어도 계속 보인다.**
- 아직 월력요항이 발표되지 않은 미래 연도는 조용히 비워 둔다(오류가 아니다).
- 계산으로 풀지 않는 이유: 설날·추석·부처님오신날은 음력이라 변환표가 필요하고,
  대체·임시공휴일은 국무회의로 정해져 규칙만으로는 맞출 수 없다
  (2025년만 해도 1/27, 6/3 임시공휴일이 있었다). 직접 계산하면 조용히 틀린다.
- 노동절·제헌절처럼 자료에는 있지만 관공서가 쉬지 않는 날은 이름만 보여 주고
  붉은빛은 주지 않는다.

### 루틴
- 운동처럼 **되풀이하는 일**은 달력에 그리지 않고 오른쪽 `루틴` 에만 모은다.
  매일 하는 일을 달력에 매일 막대로 그리면 정작 약속이 묻힌다.
- 목록 위 **`루틴`** 버튼으로 만든다. 폼도 루틴에 맞게 걷어낸다 —
  **이름 · 색 · 주기 · 요일 · 알림 · 태그**만 남는다.
  루틴은 '언제 하루' 가 아니라 '얼마마다' 가 전부라, 시작·종료 날짜와 시각·기간·중요도·링크는
  쓸 일이 없는데 자리만 차지하고 눈을 흩뜨린다. 시작일은 오늘로 조용히 잡는다. 반복(매일)과 '달력에 표시 안 함' 이 미리 켜져
  있으므로 이름만 적으면 된다. 약속과는 다른 물건이라 입구를 따로 냈다 —
  '반복 켜고 → 더보기 펼치고 → 루틴 누르고' 를 매번 거치는 건 너무 멀었다.
- 매주 반복은 **요일을 고를 수 있다** — '월수금 운동'. 고르지 않으면 시작일의 요일을 따른다.
- 체크는 회차마다 따로다. 오늘 체크해도 내일 것은 그대로 남는다.
- 그날 것을 다 하면 제목이 `루틴 · 오늘 다 했어요` 로 바뀐다.
...
- **목록이나 캘린더에서 일정을 끌어다 놓으면 고정된다.** 항목을 펼쳐 버튼을 찾을 필요가 없다.
  끄는 동안 D-Day 판이 밝아져 받을 자리라는 것을 알려 준다.
...
- 라이트 / 다크 테마, 배경 투명도·글자 크기 조절, 창 크기 프리셋.
...
### 의견 보내기
설정 아래쪽 **`메일 쓰기`** 로 기본 메일 앱이 열린다. 버전·환경이 미리 적혀 있다.

앱 안에 입력창을 두고 어딘가로 보내지 않는다. 그러려면 서버가 필요하고, 그 서버가 죽으면
사용자가 쓴 글이 조용히 사라진다. 메일 앱을 열어 주면 보낸 편지함에 남고 답장도 그대로 오간다.

받는 주소는 `app.js` 의 `FEEDBACK_TO` 한 곳에 있다.
...
- 면과 괘선은 따뜻한 회갈색이다. 미색 종이 위에 찬 회색을 얹으면 둘이 섞여 자주빛으로 읽힌다.
- **종이 질감을 흉내 내지 않는다.** 빛얼룩 그라디언트를 깔아 봤지만 종이가 아니라
  얼룩덜룩한 막처럼 보였다. 색과 글자와 여백만 남기는 편이 다이어리에 가깝다.
- **가만히 있는 화면이 기본이다.** 테두리 없는 투명 창은 한 픽셀만 바뀌어도 DWM 이 창
  전체를 다시 합성한다. D-Day 막대가 2.6초마다 숨 쉬는 것 하나에 코어 16%였다(끄면 0.1%).
  상시 애니메이션을 넣으려면 먼저 CPU 를 재고 넣을 것.
